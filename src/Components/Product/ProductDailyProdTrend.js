...
function ProductDailyProdTrend() {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.post(
        "http://172.16.200.28:3000/product-daily-prod-trend",
        {
          dateFrom: dateFrom,
          dateTo: dateTo,
          wc: selectedWC,
  ...
  useEffect(() => {
    ...
    const countColumnName_1 = columns.filter((columnName) =>
      columnName.endsWith("_1")
    ).length;
    console.log("Count 1", countColumnName_1);
    const countColumnName_2 = columns.filter((columnName) =>
      columnName.endsWith("_2")
    ).length;
    const countColumnName_3 = columns.filter((columnName) =>
      columnName.endsWith("_3")
    ).length;
    data.forEach((item) => {
                      ...
                      <table className="min-w-full divide-y divide-neutral-950 border border-slate-500 ">
                        <thead className="bg-slate-300 ">
                          ...
                          <tr className="sticky top-12 text-white z-10 bg-gray-900 whitespace-nowrap">
                            {columns.map((columnName, index) => {
                              ...
                              let bgColor =
                                index % 6 < 3 ? "#374151" : "#1F2937"; // Memeriksa apakah indeks adalah ganjil atau genap per set grup tiga kolom
                              if (columnName.endsWith("_1")) {
                                displayText = "PLAN";
                        ...
                        <tbody className="divide-y divide-neutral-950 bg-white">
                          {data?.map((item, index) => {
                            ...
                            let avgDiff = 0;
                            let avgRate = 0;
                            const countColumnName_1 = columns.filter(
                              (columnName) => columnName.endsWith("_1")
                            ).length;
                            console.log("Count 2", countColumnName_1);
                            const countColumnName_2 = columns.filter(
                              (columnName) => columnName.endsWith("_2")
                            ).length;
                            const countColumnName_3 = columns.filter(
                              (columnName) => columnName.endsWith("_3")
                            ).length;
                            return (
                              <tr key={index}>
                                ...
                                {columns.map((columnName, colIndex) => {
                                  ...
                                  avgProd = totalProd / countColumnName_2;
                                  avgDiff = totalDiff / countColumnName_3;
                                  avgRate = ((avgProd / avgPlan) * 100).toFixed(
                                    2
                                  );
                                  return (
                                    <td
                                ...
                                <td
                                  className={`whitespace-nowrap py-4 pl-4 pr-3 text-xs text-center font-medium`}
                                >
                                  {avgPlan?.toLocaleString()}{" "}
                                  {/* menampilkan total */}
                                </td>
                                <td
                                  className={`whitespace-nowrap py-4 pl-4 pr-3 text-xs text-center font-medium`}
                                >
                                  {avgProd?.toLocaleString()}{" "}
                                  {/* menampilkan total */}
                                </td>
                                <td
                                  className={`whitespace-nowrap py-4 pl-4 pr-3 text-xs text-center font-medium ${
