...
function POBalance() {
  ...
  const calculateSPKTotal = () => data.reduce((total, item) => total + (item.SPK_QTY || 0), 0);
  const calculateCuttingTotal = () => data.reduce((total, item) => total + (item.CUTTING_BL || 0), 0);
  const calculateSewingTotal = () => data.reduce((total, item) => total + (item.SEWING_BL || 0), 0);
  const calculateWhInputTotal = () => data.reduce((total, item) => total + (item.WH_BL_INPUT || 0), 0);
  const calculateWhOutTotal = () => data.reduce((total, item) => total + (item.WH_BL_OUT || 0), 0);
                  ...
                  <Combobox.Options className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                    ...
                    {filteredPOptions.map((po) => (
                      ...
                      >
                        {({ active, selected }) => (
                          <>
                            <span className={classNames('block truncate', selected && 'font-semibold')}>{po}</span>
                            {selected && (
                              <span
                                className={classNames(
                                  'absolute inset-y-0 right-0 flex items-center pr-4',
                    ...
                    <table className="min-w-full divide-y divide-neutral-950 sticky-header border border-slate-500 ">
                      <thead className="bg-slate-300">
                        ...
                        <tr>
                          ...
                          colSpan={8}
                          >TOTAL</th>
                          <th className="whitespace-nowrap py-4 pl-4 pr-3 text-xs text-center font-medium text-gray-900 sm:pl-6 bg-yellow-500">{calculateSPKTotal()}</th>
                          <th className={`whitespace-nowrap py-4 pl-4 pr-3 text-xs text-center font-medium ${calculateCuttingTotal() < 0 ? 'text-red-600 text-lg font-extrabold' : 'text-gray-900'} sm:pl-6 bg-yellow-500`}>{calculateCuttingTotal()}</th>
                          <th className={`whitespace-nowrap py-4 pl-4 pr-3 text-xs text-center font-medium ${calculateSewingTotal() < 0 ? 'text-red-600 text-lg font-extrabold' : 'text-gray-900'} sm:pl-6 bg-yellow-500`}>{calculateSewingTotal()}</th>
                          <th className={`whitespace-nowrap py-4 pl-4 pr-3 text-xs text-center font-medium ${calculateWhInputTotal() < 0 ? 'text-red-600 text-lg font-extrabold' : 'text-gray-900'} sm:pl-6 bg-yellow-500`}>{calculateWhInputTotal()}</th>
                          <th className={`whitespace-nowrap py-4 pl-4 pr-3 text-xs text-center font-medium ${calculateWhOutTotal() < 0 ? 'text-red-600 text-lg font-extrabold' : 'text-gray-900'} sm:pl-6 bg-yellow-500`}>{calculateWhOutTotal()}</th>
                        </tr>
                      </thead>
                      {updating && (
                      ...
                      <tbody className="divide-y divide-neutral-950 bg-white">
                        ...
                        {data.map(item => (
                          <tr key={item.index}>
                            ...
                            <td className="whitespace-nowrap py-4 pl-4 pr-3 text-xs text-center font-medium text-gray-900 sm:pl-6">{item.RLS}</td>
                            ...
                            <td className={`whitespace-nowrap py-4 pl-4 pr-3 text-xs text-center font-medium ${item.WH_BL_INPUT < 0 ? 'text-red-500' : 'text-gray-900'} sm:pl-6`}>{item.WH_BL_INPUT.toLocaleString()}</td>
                            <td className={`whitespace-nowrap py-4 pl-4 pr-3 text-xs text-center font-medium ${item.WH_BL_OUT < 0 ? 'text-red-500' : 'text-gray-900'} sm:pl-6`}>{item.WH_BL_OUT.toLocaleString()}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
